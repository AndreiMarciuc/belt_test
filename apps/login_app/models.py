...
class UserManager(models.Manager):
    def user_validator(self, postData): # for creating user
        ...
        if len(postData['alias'])<2: # if first name is less than 2 characters
            errors.append("Please enter your alias!")
        if not re.match("^[A-z][A-z|\.|\s]+$", postData['alias']): # if alias  contains other chars
        ...
        if len(postData['password'])<8: # check if password is longer than 8 characters
            ...
            errors.append(" Your password cannot be word 'password' be more ingenious ... !")
    ...
    def create_user(self, postData): # to create users
        ...
        return self.create(name=name, alias=alias , email=email, password=password, birthday=birthday)
...
class User(models.Model): #User class
    ...
    alias = models.CharField(max_length = 200) #stores name
